...
def dfs(current_token, balance, path, visited_edges):
    if current_token == 'tokenB' and len(path) > 1 and balance > 20:
    ...
    for (source, dis), (source_to_dis, dis_to_source) in liquidity.items():
        if source == current_token:
            if (source, dis) not in visited_edges:
                ...
                if dfs(dis, new_balance, path + [dis], visited_edges):
                    return True
            ...
            if (dis, source) not in visited_edges:
                ...
                if dfs(source, new_balance, path + [source], visited_edges):
                    return True
...
if not dfs(i_token, i_balance, i_path, visited_edges):
